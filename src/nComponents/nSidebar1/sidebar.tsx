...
import {Avatar, Spacer, Input} from "@nextui-org/react";
...
export type SidebarProps = Omit<ListboxProps<SidebarItem>, "children"> & {
  ...
  searchBar?: React.ReactNode; // Add this line
};
...
const Sidebar = React.forwardRef<HTMLElement, SidebarProps>(
  (
    ...
    return (
      <div className="relative flex h-full w-72 flex-1 flex-col border-r-small border-divider">
        <div className="bg-black p-6">
          <div className="flex items-center gap-3 px-2">
            <Avatar isBordered size="sm" src="https://i.pravatar.cc/150?u=a04258114e29026708c" />
            <div className="flex flex-col">
              <p className="text-small font-medium text-white">John Doe</p>
              <p className="text-tiny text-gray-400">AI Developer</p>
            </div>
          </div>
        </div>
          ...
          <Listbox
            ...
            items={items.map(item => {
              if (item.title === "Overview") {
                return { ...item, title: "Cortex" };
              } else if (item.title === "Organization") {
                return { ...item, title: "Synapse" };
              } else if (item.title === "Your Teams") {
                return { ...item, title: "Neurons" };
              }
              return item;
            })}
