...
const Sidebar = React.forwardRef<HTMLElement, SidebarProps>(
  (
    ...
    const handleItemClick = (key: string) => {
      const app = apps.find(app => app.id.toString() === key);
      if (app) {
        addTab(app);
      }
      onSelect?.(key);
    ...
    return (
      <Listbox
        ...
        hideSelectedIcon
        as="nav"
        className={cn("list-none bg-transparent", className)}
        classNames={{
          ...classNames,
          list: cn("items-center bg-transparent", classNames?.list),
        }}
        color="default"
        itemClasses={{
          ...itemClasses,
          base: cn(
            "px-3 min-h-11 rounded-large h-[44px] data-[selected=true]:bg-default-100 bg-transparent",
            itemClasses?.base,
          ),
          title: cn(
